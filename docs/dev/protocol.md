...
| TapElement | `0x03` | `String selector` | Tap element by accessibility ID |
| TapByLabel | `0x04` | `String label` | Tap element by accessibility label |
| TapWithType | `0x05` | `String selector`, `Bool by_label`, `String element_type` | Tap element with type constraint |
| TypeText | `0x06` | `String text` | Type text into focused element |
| Swipe | `0x07` | `i32 start_x`, `i32 start_y`, `i32 end_x`, `i32 end_y`, `Bool has_duration`, `[f64 duration if true]` | Swipe gesture with optional duration |
| GetValue | `0x08` | `String selector`, `Bool by_label`, `Optional String element_type` | Get element value |
...
To tap an element with identifier `"loginButton"`:

```
Bytes:
  [17, 0, 0, 0]       # length = 17 (1 opcode + 4 string length + 11 string bytes + 1 padding? no...)
                        # length = 1 + 4 + 11 = 16
  [16, 0, 0, 0]       # length = 16
